...
void showSpectrum()
{
  // Now we'll use a for() loop to make color count from 0 to 767
  ...
  // Each time we loop (with a new value of color), do the following:

  {
    showRGB(color);  // Call RGBspectrum() with our new x
    time = random(0,20);
    delay(time);   // Delay for 10 ms (1/100th of a second)
  }
}
